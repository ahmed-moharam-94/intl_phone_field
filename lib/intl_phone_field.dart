...
class _IntlPhoneFieldState extends State<IntlPhoneField> {
  ...
  void initState() {
    ...
    if (widget.initialCountryCode == null && number.startsWith('+')) {
      ...
      _selectedCountry = countries.firstWhere((country) =>
          number.startsWith(country.fullCountryCode),
    ...
    _maxLength = _selectedCountry.maxLength;
  }
  ...
  Widget build(BuildContext context) {
    return TextFormField(
      ...
      autofillHints: widget.disableAutoFillHints ? null : [
      ...
      onSaved: (value) {
        widget.onSaved?.call(
          PhoneNumber(
            countryISOCode: _selectedCountry.code,
            countryCode: '+${_selectedCountry.dialCode}${_selectedCountry
                .regionCode}',
            number: value!,
          ),
        );
      ...
      onChanged: (value) async {
        // check if the number starrts withs leading zero then add 1 to the length of the max phone length
        _increaseValidationLength(value: value);
      ...
      maxLength: widget.disableLengthCheck ? null : _maxLength,
  ...
  void _increaseValidationLength({required String value}) {
    if (value.startsWith('0')) {
      setState(() {
        _maxLength = _selectedCountry.maxLength + 1;
      });
    } else {
      setState(() {
        _maxLength = _selectedCountry.maxLength;
